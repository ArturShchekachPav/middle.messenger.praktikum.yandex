...
export default class UsersActions extends Action {
	...
	public changeUserAvatar(formData: FormData) {
		return this.api
			...
			.then((userData: CurrentUserType) =>
				this.store.set('currentUser.avatar', userData.avatar)
			);
	}

	public changeUserProfile(profileData: Record<string, unknown>) {
		if (
			matchesStructure<ChangeUserProfileArguments>(profileData, {
				first_name: 'string',
				second_name: 'string',
				email: 'string',
				phone: 'string',
	...
	public changeUserPassword(passwordData: Record<string, unknown>) {
		if (
			matchesStructure<ChangeUserPasswordArguments>(passwordData, {
				oldPassword: 'string',
				newPassword: 'string',
			})
		) {
